...
# Hard cap on number of chunks per document for CPU speed
...
def upload_file(request: Request, file: UploadFile = File(...), policy: str = Form(...)):
    ...
    # infer company from filename & load CSV "against" investors
    ...
    def stream():
        ...
        yield f"<p>📄 Chunked into {len(chunks)} chunks.</p>"

        if len(chunks) > MAX_CHUNKS:
            yield (
                f"<p>⚠️ Document produced {len(chunks)} chunks; "
                f"limiting to first {MAX_CHUNKS} for CPU speed.</p>"
            )
        ...
        def analyze_investor(name: str, investor_policy: str, force_reason: bool = False):
